...
impl<T: Config> Pallet<T> {
    ...
    pub fn do_register_overwatch_node_v2(
        origin: T::RuntimeOrigin,
        stake_to_be_added: u128,
    ) -> DispatchResult {
    ...
    pub fn do_set_overwatch_node_peer_id_v2(
        origin: T::RuntimeOrigin,
        subnet_id: u32,
        overwatch_node_id: u32,
        peer_id: PeerId,
    ...
    pub fn is_validator_overwatch_qualified(validator_id: u32) -> bool {
        ...
        // - No one can be an Overwatch Node yet
        if current_epoch <= min_age {
            log::error!("is_validator_overwatch_qualified 1");
            return false;
        }
        ...
        if age < min_age {
            log::error!("is_validator_overwatch_qualified 2");
            return false;
        }

        if reputation.score < min_score {
            log::error!("is_validator_overwatch_qualified 3");
            return false;
        }
        ...
        ValidatorSubnetNodes::<T>::mutate(validator_id, |node_map| {
            for (subnet_id, nodes) in node_map.iter_mut() {
                ...
                // Process each node_id one by one
                for node_id in node_ids {
                    if !Self::get_validator_subnet_node_v2(*subnet_id, node_id, subnet_epoch)
                        .is_none()
                    {
                        active_unique_node_count += 1;
                        // `break` to next subnet
        ...
        log::error!("min_diversification_ratio {:?}", min_diversification_ratio);

        if diversification < min_diversification_ratio {
            log::error!("is_validator_overwatch_qualified 4");
            return false;
        }

        if reputation.average_attestation < min_avg_attestation {
            log::error!("is_validator_overwatch_qualified 5");
            return false;
        }
